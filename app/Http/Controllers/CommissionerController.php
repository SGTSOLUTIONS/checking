    ...
    public function viewVariations($ward_no)
{
    ...
    $polygons = DB::table($polygonsTableName . ' as p')
        ...
        ->leftJoin($pointTableName . ' as pd', 'p.gisid', '=', 'pd.gisid')
        ->select(
            'p.gisid',
            'p.sqfeet',
            'pd.coordinates',
    ...
    foreach ($polygons as $index => $polygon) {
        ...
        if (!empty($coordinates)) {
            $cleanCoordinates = trim($coordinates);

            if (str_contains($cleanCoordinates, ',')) {
                $parts = explode(',', $cleanCoordinates);
    ...
    $avgVariationPercentage = $totalMisPlotArea > 0
        ? ($totalAreaVariation / $totalMisPlotArea) * 100
        : 0;
