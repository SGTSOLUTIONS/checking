...
<div class="container-fluid py-4">
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                ...
                <p class="text-muted mb-0">
                    <i class="fas fa-map-marker-alt me-2"></i>Ward {{ $ward->ward_no }} |
                    <i class="fas fa-user me-2 ms-2"></i>{{ $workerName }} |
                    <i class="fas fa-calendar me-2 ms-2"></i>{{ now()->format('d-m-Y H:i:s') }}
                </p>
            </div>
            <div class="col-md-4 text-end">
                <div class="d-inline-block px-4 py-2 bg-light rounded-3">
                    <small class="text-muted">Overall Completion</small>
                    <h3 class="mb-0 fw-bold text-primary">{{ $overallProgress['mis_completion_percentage'] ?? 0 }}%</h3>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-3 mb-3 animate-fadeInUp delay-1">
            <div class="stat-card card bg-primary text-white">
                <div class="card-body">
                    ...
                    <h2 class="mb-0 fw-bold">{{ $overallProgress['surveyed_buildings'] ?? 0 }}</h2>
                    <small class="opacity-75">Total: {{ $overallProgress['total_mis_records'] ?? 0 }}</small>
        ...
        <div class="col-md-3 mb-3 animate-fadeInUp delay-2">
            <div class="stat-card card bg-success text-white">
                <div class="card-body">
                    ...
                    <h2 class="mb-0 fw-bold">{{ $overallProgress['surveyed_points'] ?? 0 }}</h2>
                    <small class="opacity-75">Completion: {{ $overallProgress['point_completion_percentage'] ?? 0 }}%</small>
                    <div class="stat-icon-wrapper">
        ...
        <div class="col-md-3 mb-3 animate-fadeInUp delay-3">
            <div class="stat-card card bg-info text-white">
                <div class="card-body">
                    ...
                    <h2 class="mb-0 fw-bold">{{ $overallProgress['total_expected_bills'] ?? 0 }}</h2>
        ...
        <div class="col-md-3 mb-3 animate-fadeInUp delay-4">
            <div class="stat-card card bg-warning text-white">
                <div class="card-body">
                    ...
                    <h2 class="mb-0 fw-bold">{{ $overallProgress['mis_completion_percentage'] ?? 0 }}%</h2>
                    <div class="progress-modern mt-2">
                        <div class="progress-bar-modern" style="width: {{ $overallProgress['mis_completion_percentage'] ?? 0 }}%; height: 8px;"></div>
                    </div>
                    <div class="stat-icon-wrapper">
    ...
    <div class="modern-card mb-4 animate-fadeInUp">
        ...
        <div class="card-body p-0">
            @if(isset($roadWiseStats) && count($roadWiseStats) > 0)
                ...
                <table class="data-table table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="fw-bold">{{ $stat->surveyed_buildings }}</span>
                                <small class="text-muted">/ {{ $stat->total_properties }}</small>
                            </td>
                            <td>
                                <div class="d-flex justify-content-between mb-1">
                                    <small>Bills: {{ $stat->points_surveyed ?? 0 }}/{{ $stat->expected_points ?? 0 }}</small>
                                    <small class="text-muted">{{ $stat->completion_percentage }}%</small>
                                </div>
                                <div class="progress-modern">
                                    <div class="progress-bar-modern bg-success" style="width: {{ $stat->completion_percentage }}%"></div>
                                </div>
                            </td>
                            <td>
                                <h5 class="mb-0 fw-bold text-success">{{ $stat->completion_percentage }}%</h5>
                            </td>
                            <td>
                                @if($stat->completion_percentage == 100)
                                    <span class="badge-modern badge-completed"><i class="fas fa-check-circle me-1"></i>Complete</span>
                                @elseif($stat->completion_percentage >= 50)
        ...
        <div class="col-md-6 animate-fadeInUp delay-1">
            <div class="modern-card h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-3">
                        @foreach($buildingUsageStats ?? [] as $stat)
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="text-muted">{{ $stat->building_usage }}</span>
                            <div class="d-flex align-items-center gap-3">
                                <div class="progress-modern" style="width: 150px;">
                                    <div class="progress-bar-modern bg-primary" style="width: {{ ($stat->count / ($buildingUsageStats->sum('count') ?? 1)) * 100 }}%"></div>
                                </div>
    ...
    <div class="modern-card mb-4 animate-fadeInUp">
        ...
        <div class="card-body">
            <div class="row">
                @foreach($amenitiesStats ?? [] as $amenity => $stats)
                <div class="col-md-3 col-sm-6 mb-3">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body text-center">
                            <i class="fas {{ $amenity == 'liftroom' ? 'fa-arrow-up' : ($amenity == 'parking' ? 'fa-parking' : 'fa-check') }} fa-2x text-primary mb-2"></i>
                            <h6 class="text-uppercase small mb-2">{{ str_replace('_', ' ', $amenity) }}</h6>
                            <div class="d-flex justify-content-around">
                                <div>
                                    <div class="h5 mb-0 text-success">{{ $stats['yes'] }}</div>
                                    <small class="text-muted">Yes</small>
                                </div>
                                <div>
                                    <div class="h5 mb-0 text-danger">{{ $stats['no'] }}</div>
                                    <small class="text-muted">No</small>
                                </div>
                            </div>
                            <div class="progress-modern mt-2">
                                <div class="progress-bar-modern bg-success" style="width: {{ $stats['yes'] > 0 ? ($stats['yes'] / ($stats['yes'] + $stats['no'])) * 100 : 0 }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modern-card mb-4 animate-fadeInUp">
        ...
        <div class="card-body">
            <div class="row">
                @foreach($ugdStats ?? [] as $stat)
                <div class="col-md-3 col-sm-6 mb-2">
                    <div class="alert alert-info border-0 rounded-3 mb-0">
                        <i class="fas fa-plug me-2"></i>
                        <strong>{{ $stat->ugd }}</strong>
                        <span class="badge bg-light text-dark float-end">{{ $stat->count }}</span>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="table-responsive">
                <table class="data-table table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><code class="bg-light px-2 py-1 rounded">{{ $survey->gisid }}</code></td>
                            <td class="fw-semibold">{{ $survey->building_name ?? 'N/A' }}</td>
                            <td>{{ $survey->road_name ?? 'N/A' }}</td>
                            <td><span class="badge-modern bg-secondary bg-opacity-10 text-dark">{{ $survey->building_type ?? 'N/A' }}</span></td>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="progress-modern flex-grow-1" style="width: 100px;">
                                        <div class="progress-bar-modern bg-primary" style="width: {{ $survey->percentage ?? 0 }}%"></div>
                                    </div>
                                    <small class="fw-bold">{{ $survey->percentage ?? 0 }}%</small>
                                </div>
                            </td>
                            <td><i class="far fa-clock me-1 text-muted"></i>{{ \Carbon\Carbon::parse($survey->updated_at)->format('d-m-Y H:i') }}</td>
                         </tr>
                        @empty
                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        const usageLabels = @json($buildingUsageStats->pluck('building_usage'));
        const usageCounts = @json($buildingUsageStats->pluck('count'));
        ...
        const constructionLabels = @json($constructionTypeStats->pluck('construction_type'));
        const constructionCounts = @json($constructionTypeStats->pluck('count'));
