...
$(document).ready(function () {
    ...
    let currentLocationLayer = null, accuracyLayer = null, currentPosition = null;
    let locationTracking = false, watchId = null, mapCenteredOnce = false;
    ...
    function updateNavigationDisplay() {
        ...
        const remainingDistance = currentRoute.distance * (1 - (currentStepIndex / Math.max(routeSteps.length, 1)));
        const remainingTime = currentRoute.duration * (1 - (currentStepIndex / Math.max(routeSteps.length, 1)));
        ...
        $.each(allBuildings, function (i, b) {
            ...
            if (b.gisid && String(b.gisid).toLowerCase().includes(term)) {
                ...
                $.each(b.assessments, function (j, a) {
                    if (a.assessment_no && String(a.assessment_no).toLowerCase().includes(term)) {
                        match = true; type = 'Assessment No'; val = a.assessment_no; return false;
                    }
                    if (a.owner_name && String(a.owner_name).toLowerCase().includes(term)) {
                        match = true; type = 'Owner Name'; val = a.owner_name; return false;
                    }
                    if (a.phone && String(a.phone).toLowerCase().includes(term)) {
                        match = true; type = 'Phone'; val = a.phone; return false;
                    }
                });
            }
    ...
    function showPopup(gisid, coord3857) {
        ...
        $(document).off('submit', '.qc-form').on('submit', '.qc-form', function (e) {
            e.preventDefault();
            showFlashMessage('QC form submission hook ready. Connect your Laravel route here.', 'info');
        });
    }
    ...
    function createLineLayer() {
        ...
        $.each(lines, function (i, line) {
            try {
                ...
                const transformed = coords.map(c => ol.proj.fromLonLat([parseFloat(c[0]), parseFloat(c[1])]));
    ...
    function createBoundaryLayer() {
        ...
        try {
            let coords = typeof wardData.boundary === 'string' ? JSON.parse(wardData.boundary) : wardData.boundary;
            if (!coords || !coords.length) return;
    ...
    function createImageLayer() {
        if (!wardData.drone_image || !wardData.extent_left || !wardData.extent_bottom || !wardData.extent_right || !wardData.extent_top) {
            return;
        }
    ...
    function applyFilters() {
        ...
        polygonLayer.getSource().getFeatures().forEach(feature => {
            ...
            if (pd) {
                if (usage && !(pd.building_usage || '').toLowerCase().includes(usage)) visible = false;
            ...
            feature.setStyle(visible ? new ol.style.Style({
                fill: new ol.style.Fill({ color: 'rgba(255, 68, 68, 0.35)' }),
                stroke: new ol.style.Stroke({ color: '#ff4444', width: 2 })
            }) : null);
