    ...
    <div class="container-fluid p-0">
        ...
        <div class="layer-switcher panel" id="layerSwitcher">
            ...
            <div class="layer-group">
                ...
                <label><input type="checkbox" id="toggleBuildings" checked> <i class="fas fa-building"></i>
                    Buildings</label>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            async function calculateAndDisplayRoute(feature) {
                ...
                `);
                    $('.directions-list').html(
                        '<div class="text-center py-3">Route calculated! Click Google Maps for directions.</div>'
                        );
            ...
            // ==================== QC UPDATE FUNCTION ====================
            function updateAssessmentQc(assessmentId, wardNo, qcData, callback) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        showToast('Error updating QC: ' + (xhr.responseJSON?.message ||
                            'Unknown error'), 'error');
                    }
                });
            }
            ...
            function showQcModal(assessmentId, gisid, wardNo, currentQcSqfeet, currentQcUsage) {
                const modalHtml = `
                <div class="modal fade" id="qcModal" tabindex="-1" data-bs-backdrop="static">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">QC Verification - Building ${gisid}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label fw-bold">QC Square Feet</label>
                                    <input type="number" id="qcSqfeetInput" class="form-control" value="${currentQcSqfeet || ''}" placeholder="Enter verified square feet" step="any">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold">QC Usage</label>
                                    <select id="qcUsageInput" class="form-select">
                                        <option value="">Select Usage</option>
                                        <option value="Residential" ${currentQcUsage === 'Residential' ? 'selected' : ''}>Residential</option>
                                        <option value="Commercial" ${currentQcUsage === 'Commercial' ? 'selected' : ''}>Commercial</option>
                                        <option value="Industrial" ${currentQcUsage === 'Industrial' ? 'selected' : ''}>Industrial</option>
                                        <option value="Institutional" ${currentQcUsage === 'Institutional' ? 'selected' : ''}>Institutional</option>
                                        <option value="Mixed Use" ${currentQcUsage === 'Mixed Use' ? 'selected' : ''}>Mixed Use</option>
                                        <option value="Government" ${currentQcUsage === 'Government' ? 'selected' : ''}>Government</option>
                                        <option value="Vacant" ${currentQcUsage === 'Vacant' ? 'selected' : ''}>Vacant</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold">Tax Amount (₹)</label>
                                    <input type="number" id="taxAmountInput" class="form-control" placeholder="Enter tax amount" step="any">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold">Balance (₹)</label>
                                    <input type="number" id="balanceInput" class="form-control" placeholder="Enter balance" step="any">
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                <button type="button" class="btn btn-primary" onclick="saveQcAndUpdate('${assessmentId}', '${wardNo}')">Save Changes</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            window.saveQcAndUpdate = function(assessmentId, wardNo) {
                const qcSqfeet = $('#qcSqfeetInput').val();
                const qcUsage = $('#qcUsageInput').val();
                const taxAmount = $('#taxAmountInput').val();
                const balance = $('#balanceInput').val();

                updateAssessmentQc(assessmentId, wardNo, {
                    qc_sqfeet: qcSqfeet,
                    qc_usage: qcUsage,
                    ...
                    if (selectedFeature) {
                        const gisid = selectedFeature.get('gisid');
                        showPopup(gisid, selectedFeature.getGeometry().getExtent());
                    }
                });
            };
            ...
            function showPopup(gisid, coord) {
                ...
                const assessmentsHtml = !assessments.length ?
                    ...
                    assessments.map((a, i) => {
                        ...
                        return `<div class="assessment-card" data-id="${a.id || ''}" data-gisid="${pd.gisid}">
                        ...
                        <div class="assessment-body">
                            ...
                            <button class="btn btn-sm btn-primary mt-2" onclick="event.stopPropagation(); showQcModal('${a.id}', '${pd.gisid}', '{{ $ward->ward_no }}', '${a.qcsqfeet || ''}', '${a.qcusage || ''}')">
            ...
            function initMap() {
                ...
                // Add Center My Location button
                $('body').append(
                    '<button id="centerMyLocationBtn" class="center-btn" style="display:none;"><i class="fas fa-crosshairs"></i> Center to My Location</button>'
                    );
                ...
                $('#applyFilterBtn').on('click', function() {
                    ...
                    const qcStatus = $('#filterQcStatus').val();
                    ...
                    $.each(fts, function(i, f) {
                        ...
                        if (show && qcStatus !== 'all' && b) {
                            ...
                            if (qcStatus === 'completed' && !hasQC) show = false;
                            if (qcStatus === 'pending' && hasQC) show = false;
                        }
                ...
                $('#resetFilterBtn').on('click', function() {
                    $('#filterUsage, #filterQcStatus').val('all');
                ...
                $(document).on('click touchstart', function(e) {
                    if (!$(e.target).closest(
                            '.panel, .action-btn, .mobile-nav-btn, #centerMyLocationBtn, .zoom-btn, .modal, .ol-popup'
                            ).length) {
                        closeAllPanels();
                    }
                });
